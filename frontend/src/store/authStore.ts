...
import { assertPasswordIsSafe } from '../security/passwordSecurity'
...
export const useAuthStore=create<AuthState>((set,get)=>({
 ...
 signupWithEmail:async(email,password,displayName)=>{set({isLoading:true,error:null});try{await assertPasswordIsSafe(password);const{data,error}=await supabase.auth.signUp({email,password,options:{data:{display_name:displayName}}});if(error)throw error;if(!data.session){set({isLoading:false,error:'Check your email to confirm your account, then log in.'});return}await loadProfile();const updated=(await socialApi.updateProfile({displayName})).profile;set({session:data.session,profile:updated,user:authUser(data.session,updated),isAuthenticated:true,isLoading:false})}catch(error){set({isLoading:false,error:message(error)});throw error}},
